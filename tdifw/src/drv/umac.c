...
/* Write "nbytes" of zeroes, beginning at "p" */
    if (nbytes >= (int)sizeof(UWORD)) {
        while ((unsigned int)p % sizeof(UWORD)) {
            *p = 0;
            nbytes--;
            p++;
        }
...
/* Allocate memory and force to a 16-byte boundary. */
    ...
    ctx = (uhash_ctx_t)ExAllocatePoolWithTag(NonPagedPool, sizeof(uhash_ctx)+ALLOC_BOUNDARY, "camu");
    if (ctx) {
        if (ALLOC_BOUNDARY) {
            bytes_to_add = ALLOC_BOUNDARY -
                              ((unsigned int)ctx & (ALLOC_BOUNDARY -1));
 ...
 */
{
    ...
    ctx = (umac_ctx_t)ExAllocatePoolWithTag(NonPagedPool, sizeof(umac_ctx)+ALLOC_BOUNDARY, "camu");
    if (ctx) {
        if (ALLOC_BOUNDARY) {
            bytes_to_add = ALLOC_BOUNDARY -
                              ((unsigned int)ctx & (ALLOC_BOUNDARY - 1));
