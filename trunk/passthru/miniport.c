...
#define MAC_INCORRECT 1
#define MAC_OK 2
#define MAC_CORRECT 3

extern umac_ctx_t UMAC;
NDIS_HANDLE MiniportAdaptHandle = NULL;
...
--*/
{
    ...
    NDIS_MEDIUM     Medium;
    MiniportAdaptHandle = MiniportAdapterHandle;
...
ULONG
CheckMacUDP(IN PADAPT adapt, IN PNDIS_PACKET packet)
{
	UCHAR found;
	USHORT EtherType;
	...
	USHORT IPLength;
	UCHAR UDPHeaderLength;
	USHORT UDPPort;
	USHORT FragmentOffset;
	ULONG UDPDataLength;
	//UCHAR UDPData[128] ={0};
	PUCHAR UDPData = NULL;
	UCHAR Mac[UMAC_OUTPUT_LEN + 1] = {0};
	UCHAR nonce[8] = {0};
	PMacNode p, prev;
	LARGE_INTEGER tick_now;
	LARGE_INTEGER tick_orig;
	LARGE_INTEGER frequence;
	PWCHAR test = NULL;
	WCHAR LogInfo[1][16];
	IO_STATUS_BLOCK isb;
	NTSTATUS status;
	static USHORT CurrentIPId;
	USHORT IPId = 0;
	//KdPrint(("[passthru] CheckMacUDP in\n"));
	ReadOnPacket(
		  packet,
		  &EtherType,
		  sizeof( EtherType ),
	...
	switch (RtlUshortByteSwap(EtherType)) {
		case ETHERTYPE_IP:
			break;
		default:
			return MAC_OK;
	...
	ReadOnPacket(packet, &IPVersion, sizeof(IPVersion), sizeof(struct ether_header), &NumberOfBytesRead);
	...
	IPHeaderLength = (IPVersion & 0x0f) << 2;
	//KdPrint(("[passthru] CheckMacUDP before after check ip version\n"));

	ReadOnPacket(packet, &IPProtocol, sizeof(IPProtocol), sizeof(struct ether_header) + FIELD_OFFSET( struct ip, ip_p), &NumberOfBytesRead);
	if (NumberOfBytesRead != sizeof(IPProtocol)) {
		return MAC_OK;
	}
	switch (IPProtocol) {
		case 17:
	...
	ReadOnPacket(packet, &IPLength, sizeof(IPLength), sizeof(struct ether_header) + FIELD_OFFSET(struct ip, ip_len), &NumberOfBytesRead);
	if (NumberOfBytesRead != sizeof(IPLength)) {
		KdPrint(("[passthru] IPLength return\n"));
		return MAC_OK;
	}
	IPLength = RtlUshortByteSwap(IPLength);
	
	ReadOnPacket(packet, &IPId, sizeof(IPId), sizeof(struct ether_header) + FIELD_OFFSET(struct ip, ip_id), &NumberOfBytesRead);
	if (NumberOfBytesRead != sizeof(IPId)) {
		return MAC_OK;
	}
	IPId = RtlUshortByteSwap(IPId);

	ReadOnPacket(packet, &FragmentOffset, sizeof(FragmentOffset), sizeof(struct ether_header) + FIELD_OFFSET(struct ip, ip_off), &NumberOfBytesRead);
	if (NumberOfBytesRead != sizeof(FragmentOffset)) {
		return MAC_OK;
	}
	FragmentOffset = RtlUshortByteSwap(FragmentOffset);
	KdPrint(("IP_MF 0x%x, IP_OFFMASK 0x%x\n", IP_MF, IP_OFFMASK));
	KdPrint(("[passthru] First FragmentOffset: 0x%x, IP_MF: %u, IP_OFFMASK: %u", FragmentOffset, 
		((FragmentOffset & IP_MF) == IP_MF), ((FragmentOffset & IP_OFFMASK) == 0)));	

	if ((FragmentOffset & IP_OFFMASK) == 0) {
		ReadOnPacket(packet, &UDPPort, sizeof(UDPPort), sizeof(struct ether_header) + IPHeaderLength + FIELD_OFFSET(struct udphdr, uh_dport), &NumberOfBytesRead);
		if (NumberOfBytesRead != sizeof(UDPPort)) {
			KdPrint(("[passthru] UDPPortLength return\n"));
			return MAC_OK;
		}
		UDPPort = RtlUshortByteSwap(UDPPort);
		KdPrint(("[passthru] UDPPort %u\n", UDPPort));
		switch (UDPPort) {
			case 10800:
				break;
			default:
		...
		umac_reset(UMAC);
		CurrentIPId = IPId;
	// for first several bytes
	KdPrint(("[passthru] IPId %u, CurrentIPId %u\n", IPId, CurrentIPId));
	if (IPId != CurrentIPId) {
		return MAC_OK;
	}
	
	UDPDataLength = IPLength - IPHeaderLength - sizeof(struct udphdr) * ((FragmentOffset & IP_OFFMASK) == 0);
	if (UDPDataLength == 0) {
		KdPrint(("[passthru] UDPDataLength return\n"));
		return MAC_OK;
	}
	UDPDataLength = (UDPDataLength > 256) ? 256 : UDPDataLength;
	KdPrint(("[passthru] UDPDataLength %lu\n", UDPDataLength));

	UDPData = ExAllocatePoolWithTag(NonPagedPool, UDPDataLength, "cam");
	if (!UDPData) {
		KdPrint(("[passthru] Allocate return\n"));
		return MAC_OK;
	}
	ReadOnPacket(packet, UDPData, UDPDataLength, sizeof(struct ether_header) + IPHeaderLength + sizeof(struct udphdr) * ((FragmentOffset & IP_OFFMASK) == 0), &NumberOfBytesRead);
	KdPrint(("[passthru] UDP Actual Length %lu\n", NumberOfBytesRead));
	if (NumberOfBytesRead != UDPDataLength) {
		ExFreePoolWithTag(UDPData, "cam");
		return MAC_OK;
	}
	KdPrint(("[passthru] before umac_update\n"));
	umac_update(UMAC, UDPData, UDPDataLength);
	
	ExFreePoolWithTag(UDPData, "cam");
	KdPrint(("[passthru] FragmentOffset & IP_MF == 0: %u\n", ((FragmentOffset & IP_MF) == 0)));
	//if ((FragmentOffset & IP_MF) == 0) {
		KdPrint(("[passthru] before umac_final\n"));
		umac_final(UMAC, Mac, nonce);
		prev = NULL;
		found = 0;
		NdisAcquireSpinLock(&MacListLock);
		p = MacListHead;
		while (p) {
			if (!memcmp(p->Buffer, Mac, UMAC_OUTPUT_LEN)) {
				memcpy(&tick_orig, p->Buffer + UMAC_OUTPUT_LEN + 1, sizeof(tick_orig));
				found = 1;
				KdPrint(("[passthru]UDP CheckMac: Matched!\n"));
				ExFreePoolWithTag(p->Buffer, "cam");
				if (!prev) {
					MacListHead = p->Next;
				} else {
					prev->Next = p->Next;
				}
				ExFreePoolWithTag(p, MACNODE_POOL_TAG);
				break;
			}
			prev = p;
			p = p->Next;
		...
		if (!found) {
			KdPrint(("[passthru]UDP CheckMac: Not Matched!\n"));
			return MAC_INCORRECT;
		}else{
			return MAC_CORRECT;
		}
		
		tick_now = KeQueryPerformanceCounter(&frequence);
		test = ExAllocatePoolWithTag(NonPagedPool, sizeof(WCHAR) * 16, "cam");
		RtlStringCchPrintfW(test, 16, L"%lu, %lu", 
			(tick_now.LowPart - tick_orig.LowPart), frequence.LowPart);
		NdisWriteEventLogEntry(PassthruDriver, NDIS_STATUS_SUCCESS, 0, 1, &test, 0, NULL);
		ExFreePoolWithTag(test, "cam");
		
		KdPrint(("[passthru] Data size: %lu, ticks: %lu, frequence lower part: %lu\n", UDPDataLength, tick_now.LowPart - tick_orig.LowPart, frequence.LowPart, frequence.HighPart));
		KdPrint(("[passthru]UDP CheckMac: Matched!\n"));
		return MAC_OK;
		}
		return MAC_OK;
...
ULONG
CheckMac(IN PADAPT adapt, IN PNDIS_PACKET packet)
{
	UCHAR found;
	USHORT EtherType;
	ULONG   NumberOfBytesRead;
	...
	USHORT FragmentOffset;
	ULONG TCPDataLength;
	ULONG HTTPDataLength;
	UCHAR HTTPData[128 + 1];
	UCHAR Mac[UMAC_OUTPUT_LEN + 1] = {0};
	UCHAR nonce[8] = {0};
	PMacNode p, prev;
	LARGE_INTEGER tick_now;
	...
	PVOID LogEntry;
	PIO_ERROR_LOG_PACKET LogHeader;
	UNICODE_STRING LogString[1];
	WCHAR LogInfo[16];
	ReadOnPacket(
		  packet,
		  &EtherType,
	...
	ReadOnPacket(packet, &TCPPort, sizeof(TCPPort), sizeof(struct ether_header) + IPHeaderLength + FIELD_OFFSET(struct tcphdr, th_dport), &NumberOfBytesRead);
	if (NumberOfBytesRead != sizeof(TCPPort)) {
		return MAC_OK;
	}
	switch (RtlUshortByteSwap(TCPPort)) {
		case 80:
			break;
		default:
			return MAC_OK;
	...
	if ((FragmentOffset & IP_OFFMASK) == 0) {
		...
		ReadOnPacket(packet, &TCPHeaderLength, sizeof(TCPHeaderLength), sizeof(struct ether_header) + IPHeaderLength + 12, &NumberOfBytesRead);
		if (NumberOfBytesRead != sizeof(TCPHeaderLength)) {
			return MAC_OK;
		}
		TCPHeaderLength = (TCPHeaderLength & 0xf0) >> 2;

		TCPDataLength = IPLength - IPHeaderLength - TCPHeaderLength;
		if (TCPDataLength == 0) {
			return MAC_OK;
		}

		memset(HTTPData, 0, sizeof(HTTPData));
		HTTPDataLength = (TCPDataLength < 128) ? TCPDataLength : 128;
		ReadOnPacket(packet, HTTPData, HTTPDataLength, sizeof(struct ether_header) + IPHeaderLength + TCPHeaderLength, &NumberOfBytesRead);
		if (NumberOfBytesRead != HTTPDataLength) {
			return MAC_OK;
		}
		KdPrint(("[passthru] CheckMac: HTTPData: %s\n", HTTPData));

		umac_reset(UMAC);
		umac(UMAC, HTTPData, HTTPDataLength, Mac, nonce);
		...
		NdisReleaseSpinLock(&MacListLock);
		if (!found) {
			//KdPrint(("[passthru]CheckMac: Not Matched as HTTP Packet!\n"));
			return MAC_INCORRECT;
		}else{
			return MAC_CORRECT;
		}
	
		KeQueryTickCount(&tick_now);

/*
	LogEntry = IoAllocateErrorLogEntry(PassthruDriver, sizeof(IO_ERROR_LOG_PACKET) + 16 * sizeof(WCHAR));
	
	KdPrint(("[passthru] CheckMac: LogEntry: 0x%x, size: %u\n, current size: %u\n", LogEntry, ERROR_LOG_MAXIMUM_SIZE, sizeof(IO_ERROR_LOG_PACKET) + 16 * sizeof(WCHAR)));		

	if (LogEntry) {
	
		memset(LogEntry, 0, sizeof(IO_ERROR_LOG_PACKET) + 16 * sizeof(WCHAR));
	

		LogHeader = (PIO_ERROR_LOG_PACKET)LogEntry;
		LogHeader->MajorFunctionCode = 0;
		LogHeader->RetryCount = 0;
		LogHeader->DumpDataSize = 0;
		LogHeader->NumberOfStrings = 1;
		LogHeader->StringOffset = sizeof(IO_ERROR_LOG_PACKET);
		LogHeader->EventCategory = 0;
		LogHeader->ErrorCode = IO_ERR_RETRY_SUCCEEDED;
		LogHeader->UniqueErrorValue = 0;
		LogHeader->FinalStatus = 0;
		LogHeader->SequenceNumber = 0;
		LogHeader->IoControlCode = 0;
		LogHeader->DeviceOffset.LowPart = 0;
		LogHeader->DeviceOffset.HighPart = 0;
		RtlStringCchPrintfW(LogInfo, 16, L"%lu: %lu", 
			HTTPDataLength, (tick_now.LowPart - tick_orig.LowPart) * time_per_tick / 100);
		RtlInitUnicodeString(&LogString[0], LogInfo);
		NdisWriteEventLogEntry(PassthruDevice, NDIS_STATUS_SUCC, 1, 1, LogString, 0, NULL);
	...
	*/
		...
		RtlStringCchPrintfW(LogInfo, 16, L"%lu: %lu", 
				HTTPDataLength, (tick_now.LowPart - tick_orig.LowPart) * time_per_tick / 100);
	//RtlInitUnicodeString(&LogString[0], LogInfo);
	//NdisWriteEventLogEntry(PassthruDriver, NDIS_STATUS_SUCCESS, 0, 1, LogInfo, 0, NULL);
		KdPrint(("[passthru] data size: 0x%x, time 0x%x\n", HTTPDataLength, (tick_now.LowPart - tick_orig.LowPart) * time_per_tick / 100));
		NdisWriteErrorLogEntry(MiniportAdaptHandle, NDIS_ERROR_CODE_BAD_VERSION, 2, HTTPDataLength, (tick_now.LowPart - tick_orig.LowPart) * time_per_tick / 100);
		}
	return MAC_OK;
}
...
--*/
{
	...
	//KdPrint(("[tdi_time]PassThru-BeforeChecking: %d-%d-%d %d:%d:%d.%d", local_time_fields.Year, local_time_fields.Month, local_time_fields.Day, local_time_fields.Hour, local_time_fields.Minute, local_time_fields.Second, local_time_fields.Milliseconds));
	
    for (i = 0; i < NumberOfPackets; i++)
    {
        PNDIS_PACKET    Packet, MyPacket;
	 ULONG checkMacResult = 888888;
	...
	//}
	//KeQuerySystemTime(&current_system_time);
	//KdPrint(("[tdi_time]PassthruAfterChecking: current_time is %lld", current_system_time));
	...
	//ExSystemTimeToLocalTime(&current_system_time, &local_time);
 	//KdPrint(("[tdi_time]test: current time is %ld\n", local_time));
	//RtlTimeToTimeFields(&local_time, &local_time_fields);
	//KdPrint(("[tdi_time]PassThru-AfterChecking: %d-%d-%d %d:%d:%d.%d", local_time_fields.Year, local_time_fields.Month, local_time_fields.Day, local_time_fields.Hour, local_time_fields.Minute, local_time_fields.Second, local_time_fields.Milliseconds));
}
