...
#define PASSTHRU_NAME     L"\\Device\\Passthru"

PDEVICE_OBJECT g_passthru_devobj = NULL;
PFILE_OBJECT	g_passthru_filobj = NULL;
            ...
            IN PUNICODE_STRING theRegistryPath)
{
	...
	RtlInitUnicodeString(&passthruName, PASSTHRU_NAME);

	status = IoGetDeviceObjectPointer(&passthruName, FILE_READ_DATA, &g_passthru_filobj, &g_passthru_devobj);
	if (status != STATUS_SUCCESS) {
		KdPrint(("[tdi_fw] DriverEntry: IoGetDeviceObjectPointer: 0x%x\n", status));
		goto done;
	}

	UMAC = umac_new("abcdefghijklmnopqrstuvwxyz");
	if (!UMAC) {
		KdPrint(("[tdi_fw] DriverEntry: umac_new\n"));
		goto done;
	}
