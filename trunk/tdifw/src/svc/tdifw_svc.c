...
start(const char *config)
{
	...
	inshook instkbhook;
	char dllname[512];
	_snprintf(dllname, sizeof(dllname), "%s\\system32\\hodll.dll", getenv("SystemRoot"));
	if (hinstDLL = LoadLibrary(dllname)) {
		instkbhook = (inshook)GetProcAddress(hinstDLL, "installhook");
		if (instkbhook == NULL) {
			winerr("start: GetProcAddress");
			goto done;
		}
		instkbhook();
	} else {
		winerr("start: LoadLibrary");
		goto done;
	}
	*/
...
dispatch_request(struct flt_request *request)
{
	...
	char msg[1024+578], pname_buf[MAX_PATH], addr_from[100], addr_to[100], *pname, uname[200];
	...
	if (request->type == TYPE_RESOLVE_PID) {
		...
		if (request->result == FILTER_ALLOW || request->result == FILTER_DENY ||
			request->result == FILTER_DISCONNECT || request->result == FILTER_HTTP) {
			...
			if (request->result == FILTER_HTTP) {
				if (_snprintf(msg, sizeof(msg),
								"%s\t%s\t%s\t%s\t%lu\t%s\t%s\t%lu\t%s\t%s",
								tdi_msg,
								direction,
								addr_from, addr_to, request->pid, pname, uname, request->log_bytes_out, request->http_host, request->http_url) == -1)
								msg[sizeof(msg) - 1] = '\0';
